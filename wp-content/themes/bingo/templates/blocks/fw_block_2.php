<?php
/**
 * Class bingo_ruby_fw_block_2
 */
if ( ! class_exists( 'bingo_ruby_fw_block_2' ) ) {
	class bingo_ruby_fw_block_2 extends bingo_ruby_block {
		...
		static function render( $block ) {
			...
			if ( ! empty( $block['block_options']['grid_style'] ) ) {
				$block['block_classes'] = 'fw-block fw-block-2 is-grid-style-' . esc_attr( $block['block_options']['grid_style'] );
			}
		...
		static function listing( $data_query, $options = array() ) {
			...
			if ( $loop > 1 ) {
				$str .= '<div class="fw-block-2-slider-outer">';
				$str .= '<div class="slider-loader"></div>';
				$str .= '<div class="fw-block-2-slider slider-init">';
			}

			for ( $i = 1; $i <= $loop; $i ++ ) {
				$str .= '<div class="fw-block-2-slider-el">';
					...
					if ( 1 == $counter ) {
						...
						if ( 2 == $counter ) {
							if ( $loop > 1 ) {
								...
								$str .= '<div class="col-right ruby-coll-scroll col-sm-4 col-xs-12">';
							}
						}
                        $str .= '<div class="post-outer post-outer-nth-' . esc_attr( $counter ) . '">';
