<?php
/**
 * Class bingo_ruby_hs_block_1
 */
if ( ! class_exists( 'bingo_ruby_hs_block_1' ) ) {
	class bingo_ruby_hs_block_1 extends bingo_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes'] = 'hs-block hs-block-1 block-bg-full';
		...
		static function listing( $data_query, $options = array() ) {
			...
			while ( $data_query->have_posts() ) {
				...
				if ( 1 == $counter ) {
					...
					$str .= bingo_ruby_post_grid_3( $options );
					$str .= '</div><!--#left column-->';
				} else {
					if ( 2 == $counter ) {
