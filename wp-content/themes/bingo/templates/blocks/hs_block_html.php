<?php
/**
 * Class bingo_ruby_hs_block_html
 * render has sidebar block custom html
 */
if ( ! class_exists( 'bingo_ruby_hs_block_html' ) ) {
	class bingo_ruby_hs_block_html extends bingo_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes'] = 'hs-block hs-block-html';
		...
		static function content( $block ) {
			...
			if ( ! empty( $block['block_options']['custom_html'] ) ) {
				...
				//render html
				$content = html_entity_decode( stripslashes( $block['block_options']['custom_html'] ) );
