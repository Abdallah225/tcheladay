<?php
/**
 * Class bingo_ruby_fw_block_g3
 */
if ( ! class_exists( 'bingo_ruby_fw_block_g3' ) ) {
	class bingo_ruby_fw_block_g3 extends bingo_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes'] = 'fw-block fw-block-g3 block-bg-full';
		...
		static function listing( $data_query, $options = array() ) {
			...
			while ( $data_query->have_posts() ) {
				...
				$str .= '<div class="post-outer col-sm-3 col-xs-12">';
				$str .= bingo_ruby_post_list_m1( $options );
				$str .= '</div><!--#post outer-->';
			}
